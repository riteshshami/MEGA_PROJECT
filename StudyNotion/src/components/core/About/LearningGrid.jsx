...
const LearningGrid = () => {
  return (
    <div className="grid mx-auto grid-col-1 lg:grid-cols-4 mb-10 mt-[90px]">
    {
        LearningGridArray.map((card, index) => (
            <div 
            className={`${index === 0 && "lg:col-span-2 lg:h-[250px] bg-richblack-900"}
            ...
            key={index}>
            {
                card.order < 0 ? (
                    <div className="lg:w-[90%] flex flex-col pb-5 gap-3">
                        ...
                        <div className="w-fit">
                        <CTAbutton active={true} linkto={card.link}>{card.BtnText}</CTAbutton>
                        </div>
                    </div>
                ) : (
